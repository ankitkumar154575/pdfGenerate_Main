...
export class WorkItemComponent implements OnInit {
  ...
  CitiCMT_Function() {
    ...
    const redFlagDetails = this.selectedWorkItem.summary[0].redFlags.map(
      (redFlag: any, index: number) => [redFlag.redFlagCode, redFlag.redFlag]
    );

    // Add table headers
    const headers = [['S No.', 'Red Flags']];
    startY = this.addCustomTable(doc, headers.concat(redFlagDetails), startY);
    addSpacing(8);
  ...
  // Function to add a custom table with two columns: S No. and Red Flags
  addCustomTable(doc: jsPDF, data: any[][], startY: number): number {
    const rowHeight = 10;
    const colWidth = 48;
    const startX = 10;
    let currentY = startY;

    // Loop through the data and create rows for "S No." and "Red Flags"
